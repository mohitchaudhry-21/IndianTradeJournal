...
export function JournalProvider({ children }) {
  ...
  const addTrades = useCallback((newTrades) => {
    // Bulk import (broker sync) — upsert by brokerTradeId (update if exists, add if new)
    setTrades(prev => {
      const existingById = {};
      ...
      newTrades.forEach(t => {
        ...
        if (t.brokerTradeId && existingById[t.brokerTradeId]) {
          // Update existing trade in place (preserves notes/tags but updates prices)
...
export function JournalProvider({ children }) {
  ...
  const addTrades = useCallback((newTrades) => {
    // Bulk import (broker sync) — deduplicate by brokerTradeId
    setTrades(prev => {
      const existingIds = new Set(prev.map(t => t.brokerTradeId).filter(Boolean));
      const fresh = newTrades
        .filter(t => !t.brokerTradeId || !existingIds.has(t.brokerTradeId))
        .map(t => ({ ...t, id: t.id || uuidv4(), createdAt: new Date().toISOString() }));
