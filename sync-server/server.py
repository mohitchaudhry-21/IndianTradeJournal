...
def sync_angelone():
    ...
    try:
        ...
        broker_open = [t for r in positions_raw if (t := map_angel_position(r, account_id)) is not None]
        broker_open = group_open_positions(broker_open)

        # Get tradebook to find exit prices for closed positions
        tradebook = angel_tradebook(s['apiKey'], s['jwtToken'])

        # Build a map of broker open positions by symbol key
        broker_keys = set()
        for pos in broker_open:
            for leg in pos.get('legs', [pos]):
                key = (leg.get('instrument',''), str(leg.get('expiry',''))[:10], str(leg.get('strike','')), leg.get('optionType',''))
                broker_keys.add(key)

        # Find journal positions to CLOSE (open in journal but gone from broker)
        existing_open = data.get('existingPositions', [])
        to_close = []
        for ep in existing_open:
            legs = ep.get('legs', [])
            pos_id = ep.get('positionId', '')
            # Check if any leg is still open in broker
            still_open = any(
                (leg.get('instrument',''), str(leg.get('expiry',''))[:10],
                 str(leg.get('strike','')), leg.get('optionType','')) in broker_keys
                for leg in legs
            )
            if not still_open and pos_id:
                # Find exit prices from tradebook
                exit_legs = []
                for leg in legs:
                    sym = leg.get('instrument','') + leg.get('optionType','')
                    exit_price = 0
                    for tb in tradebook:
                        tb_sym = parse_symbol(tb.get('tradingsymbol', ''))
                        if (tb_sym and
                            tb_sym.get('instrument') == leg.get('instrument') and
                            str(tb_sym.get('expiry',''))[:10] == str(leg.get('expiry',''))[:10] and
                            str(tb_sym.get('strike')) == str(leg.get('strike')) and
                            tb_sym.get('optionType') == leg.get('optionType')):
                            # Exit order = opposite of entry
                            entry_tx = leg.get('transactionType', 'SELL')
                            exit_tx = 'BUY' if entry_tx == 'SELL' else 'SELL'
                            if tb.get('transactiontype', '').upper() == exit_tx:
                                exit_price = float(tb.get('price', 0) or tb.get('averageprice', 0) or 0)
                    exit_legs.append({'legId': leg.get('id'), 'exitPrice': exit_price})
                to_close.append({'positionId': pos_id, 'exitDate': datetime.now().strftime('%Y-%m-%d'), 'exitLegs': exit_legs})

        return jsonify({
            'success': True,
            'count': len(broker_open),
            'trades': broker_open,
...
if __name__ == '__main__':
    ...
    print('=' * 55)
    app.run(host='0.0.0.0', port=5001, debug=False)# ── Symbol parser ─────────────────────────────────────────────────────────────
# Handles formats:  BANKNIFTY14MAY2548000CE  /  NIFTY25500CE25APR25
MONTH_MAP = {'JAN':1,'FEB':2,'MAR':3,'APR':4,'MAY':5,'JUN':6,
              'JUL':7,'AUG':8,'SEP':9,'OCT':10,'NOV':11,'DEC':12}
...
def map_angel_position(p, account_id):
    ...
    return {
        ...
        'lotSize': lot_size,
        'premium': premium,
        'date': (p.get('orderplacetime') or p.get('time') or '')[:10] or datetime.now().strftime('%Y-%m-%d'),
        'status': 'OPEN',
        'strategyName': 'Custom',
    }
